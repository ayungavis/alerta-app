...
final class SoundAnalysisClassifier: NSObject, SNResultsObserving {
    ...
    init(format: AVAudioFormat,
         ...
         onResult: @escaping (SoundClassificationMatch) -> Void) throws
    {
        self.analyzer = SNAudioStreamAnalyzer(format: format)
        self.request = try SNClassifySoundRequest(classifierIdentifier: .version1)
        self.analysisQueue = DispatchQueue(label: "sound-analysis-classifier")
    ...
    private func mapIdentifierToSpecificEvent(_ identifier: String) -> SoundEvent? {
        ...
        if containsAny(label, keywords: ["car", "truck", "bus", "vehicle", "motorcycle", "motor vehicle", "engine", "traffic"]) {
