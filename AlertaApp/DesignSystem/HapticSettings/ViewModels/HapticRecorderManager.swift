...
class HapticRecorderManager {
    ...
    func touchUp() {
        ...
        if let lastUp = lastTouchUpTime {
            waitTime = downTime.timeIntervalSince(lastUp)
        } else {
            waitTime = 0.0
        }
    ...
    func playCustomPattern(steps: [RecordedStep]) {
        ...
        for step in steps {
            ...
            let event = CHHapticEvent(eventType: eventType, parameters: [intensity, sharpness], relativeTime: currentTime, duration: step.duration)
            events.append(event)
